...
fichier_both = 'sync_data_2018-08-15_both.csv'
fichier_felix = 'sync_data_2018-08-15_felix.csv'
fichier_felix_interp = 'sync_data_2018-08-15_felix_interp.csv'
fichier_sebastien = 'sync_data_2018-08-15_sebastien.csv'
...
fichier_garmin='Garmin_20180815.csv'
fichier_both = 'sync_data_2018-08-17_both.csv'
fichier_felix = 'sync_data_2018-08-17_felix.csv'
fichier_felix_interp = 'sync_data_2018-08-17_felix_interp.csv'
fichier_sebastien = 'sync_data_2018-08-17_sebastien.csv'
...
fichier_both = 'sync_data_2018-08-27_both.csv'
fichier_felix = 'sync_data_2018-08-27_felix.csv'
fichier_felix_interp = 'sync_data_2018-08-27_felix_interp.csv'
fichier_sebastien = 'sync_data_2018-08-27_sebastien.csv'
...
fichier_both = 'sync_data_2018-08-30_both.csv'
fichier_felix = 'sync_data_2018-08-30_felix.csv'
fichier_felix_interp = 'sync_data_2018-08-30_felix_interp.csv'
fichier_sebastien = 'sync_data_2018-08-30_sebastien.csv'
...
for ll in range(1, nbl):
    ...
    Picarro_time.append(datetime(year,month,day,hour,minute,second)+Picarro_shift)
    Picarro_time_epoch.append( (datetime(year,month,day,hour,minute,second) - datetime(1970,1,1)).total_seconds() )
...
for ii in range(1,len(lat_garmin)):
	...
	'''
	distance = R * degToRad * math.sqrt(math.pow(math.cos(lat_garmin[ii-1] * degToRad ) * (lon_garmin[ii-1] - lon_garmin[ii]) , 2) + math.pow(lat_garmin[ii-1] - lat_garmin[ii], 2))
	temps = garmin_time[ii]-garmin_time[ii-1]
	car_s_garmin.append( distance / temps.total_seconds())
	
	X = math.cos(lat_garmin[ii]* degToRad) * math.sin((lon_garmin[ii]-lon_garmin[ii-1])* degToRad)
	Y = math.cos(lat_garmin[ii-1]* degToRad) * math.sin(lat_garmin[ii]* degToRad) - math.sin(lat_garmin[ii-1]* degToRad) * math.cos(lat_garmin[ii]* degToRad) * math.cos((lon_garmin[ii] - lon_garmin[ii-1])* degToRad)
	if math.atan2( X , Y ) * 180 / math.pi >= 0:
		car_d_garmin.append(math.atan2( X , Y ) * 180 / math.pi)
	if math.atan2( X , Y ) * 180 / math.pi < 0:
		car_d_garmin.append(math.atan2( X , Y ) * 180 / math.pi + 360)
	'''
...
for ii in range(0,len(meteo_time)):
	for ll in range(0,len(garmin_time_w)):
			if meteo_time[ii].day == garmin_time_w[ll].day and meteo_time[ii].hour == garmin_time_w[ll].hour and meteo_time[ii].minute == garmin_time_w[ll].minute and meteo_time[ii].second == garmin_time_w[ll].second:
				#wind_direction_garmin_corr.append(math.atan( (wind_speed[ii]*math.cos(math.radians(wind_direction[ii])) - car_s_garmin[ll]*math.cos(math.radians(car_d_garmin[ll]))) / (wind_speed[ii]*math.sin(math.radians(wind_direction[ii])) - car_s_garmin[ll]*math.sin(math.radians(car_d_garmin[ll]))) ))
...
if os.path.exists(fichier_both):		# if file exists delete 
  os.remove(fichier_both)
if os.path.exists(fichier_felix):		# if file exists delete 
  os.remove(fichier_felix)
if os.path.exists(fichier_felix_interp):		# if file exists delete 
  os.remove(fichier_felix_interp)
if os.path.exists(fichier_sebastien):		# if file exists delete 
  os.remove(fichier_sebastien)
if os.path.exists(fichier_sebastien_interp):		# if file exists delete 
  os.remove(fichier_sebastien_interp)

nom_both = open(fichier_both, "w")
nom_felix = open(fichier_felix, "w")
nom_felix_interp = open(fichier_felix_interp, "w")
nom_sebastien = open(fichier_sebastien, "w")
nom_sebastien_interp = open(fichier_sebastien_interp, "w")

nom_both.write('gps_time,alt,amb,ch4,ch4d,co,co2,co2d,cod,cog,gasp,h2o,hdop,heading,lat,lon,pressure,rd1,rd2,sog,t,temp,wd_corr,wd_uncorr,ws_corr,ws_uncorr\n')
nom_felix.write('gps_time,alt,amb,ch4,ch4d,co,co2,co2d,cod,cog,gasp,h2o,hdop,heading,lat,lon,pressure,rd1,rd2,sog,t,temp,wd_corr,wd_uncorr,ws_corr,ws_uncorr\n')
...
nom_sebastien.write('gps_time,alt,amb,ch4,ch4d,co,co2,co2d,cod,cog,gasp,h2o,hdop,heading,lat,lon,pressure,rd1,rd2,sog,t,temp,wd_corr,wd_uncorr,ws_corr,ws_uncorr\n')
...
for ii in range(0,len(Picarro_time)):
	...
	n = 0
	for jj in range(0,len(gps_time)):
		if Picarro_time[ii].day == gps_time[jj].day and Picarro_time[ii].hour == gps_time[jj].hour and Picarro_time[ii].minute == gps_time[jj].minute and Picarro_time[ii].second == gps_time[jj].second:				
			print 'GPS'
			nom_both.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (gps_time[jj],alt[jj],'amb[ii]',ch4[ii],ch4d[ii],'co[ii]',co2[ii],co2d[ii],'cod[ii]','cog[jj]','gasp[ii]',h2o[ii],'hdop[jj]','heading[jj]',lat[jj],lon[jj],pressure,'rd1[ii]','rd2[ii]','sog[jj]','t[ii]',temp,'wd_corr[jj]',wd_uncorr,'ws_corr[jj]',ws_uncorr))
			nom_felix.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (gps_time[jj],alt[jj],'amb[ii]',ch4[ii],ch4d[ii],'co[ii]',co2[ii],co2d[ii],'cod[ii]','cog[jj]','gasp[ii]',h2o[ii],'hdop[jj]','heading[jj]',lat[jj],lon[jj],pressure,'rd1[ii]','rd2[ii]','sog[jj]','t[ii]',temp,'wd_corr[jj]',wd_uncorr,'ws_corr[jj]',ws_uncorr))
			n = n + 1				
			break

	if n == 0:
			for kk in range(0,len(garmin_time)):
				if Picarro_time[ii].day == garmin_time[kk].day and Picarro_time[ii].hour == garmin_time[kk].hour and Picarro_time[ii].minute == garmin_time[kk].minute and Picarro_time[ii].second == garmin_time[kk].second:
					print 'GARMIN'
					nom_both.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (garmin_time[kk],alt_garmin[kk],'amb[ii]',ch4[ii],ch4d[ii],'co[ii]',co2[ii],co2d[ii],'cod[ii]','cog[jj]','gasp[ii]',h2o[ii],'hdop[jj]','heading[jj]',lat_garmin[kk],lon_garmin[kk],pressure,'rd1[ii]','rd2[ii]','sog[jj]','t[ii]',temp,'wd_corr[jj]',wd_uncorr,'ws_corr[jj]',ws_uncorr))
					break
	...
	for kk in range(0,len(garmin_time)):
		if Picarro_time[ii].day == garmin_time[kk].day and Picarro_time[ii].hour == garmin_time[kk].hour and Picarro_time[ii].minute == garmin_time[kk].minute and Picarro_time[ii].second == garmin_time[kk].second:
			print 'GARMIN'
			nom_sebastien.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (garmin_time[kk],alt_garmin[kk],'amb[ii]',ch4[ii],ch4d[ii],'co[ii]',co2[ii],co2d[ii],'cod[ii]','cog[jj]','gasp[ii]',h2o[ii],'hdop[jj]','heading[jj]',lat_garmin[kk],lon_garmin[kk],pressure,'rd1[ii]','rd2[ii]','sog[jj]','t[ii]',temp,'wd_corr[jj]',wd_uncorr,'ws_corr[jj]',ws_uncorr))
			break
